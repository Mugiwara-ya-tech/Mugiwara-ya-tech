...
Still to fill in by hand in `README.md`: nothing required — socials are all set. The
portrait comes from `assets/jacket.png` (the cutout photo); to change the look, swap
that file and re-run dotify (see below).
...
`assets/jacket.png` is not the raw photo — it is the photo with a brightness
lift already applied (`L' = 255*(L/255)^0.38`, per-pixel channel scaling, alpha
preserved), because the original is a very dark edit whose dots would disappear
against GitHub's dark background. Run dotify on `jacket.png` as-is to reproduce
this portrait; if you swap in a new photo of normal brightness you can skip the
lift entirely.
...
```powershell
python scripts\cards.py --user Mugiwara-ya-tech --out assets
```
